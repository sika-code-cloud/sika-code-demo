...
public class AuthController extends BaseLiteflowServerController {
    ...
    public Result response(@RequestBody MigrateResultDTO resultDTO) {
        log.info("请求的参数为：{}", JSON.toJSONString(resultDTO.getResult()));
        log.info("请求的头为：{}", JSON.toJSONString(resultDTO.getHeader()));
        MigrateRuleResult ruleResult = new MigrateRuleResult();
        SpringMVCUtil.getRequest().setAttribute(MigrateConstant.REQUEST_ID_KEY, resultDTO.getRequestId());
        SpringMVCUtil.getRequest().setAttribute(MigrateConstant.MIGRATE_TYPE_KEY, MigrateTypeEnum.CONTRAST.getType());
        SpringMVCUtil.getRequest().setAttribute("bodyFrom", "client");
        ruleResult.setResult(resultDTO.getResult());
